...
uniform sampler2D clothingAlpha;
uniform vec4 clothingAlphaScaleOffset;
...
void main() {
	...
	float scaledValue = vertexColor.a * 2.0;
	...
	color.a *= texture(alphaMask, uv * alphaMaskScaleOffset.xy + alphaMaskScaleOffset.zw).r * alpha1weight + texture(clothingAlpha, uv * clothingAlphaScaleOffset.xy + clothingAlphaScaleOffset.zw).r * alpha2weight + alpha0weight;
	color.a *= ComputeMask(baseMaskType, baseMaskParameters, baseMaskAxis, tangentTransform, worldNormal, surfaceNormal);
	fragmentColor = mix(color, color * vec4(0.6, 0.6, 0.6, 1.f), vertexColor.r);
}